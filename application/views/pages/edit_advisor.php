<div class="content">
	<div class="left_col">
		<img src="/images/no_profile_icon2.png" width=200 height=200 alt="profile image"/>
		<button>Clear Image</button>
		<button>Upload an Image</button>
	</div><!--left column-->
	
	<div class="right_col">
		<div class="info">
			<label class="title">Name:</label><label>Madalyn Coryea</label><br/>
			<label class="title">Department:</label><label>Computer Science</label><br/>
			<label class="title">Gender:</label>
				<input name="gender" type="radio" /><label>Female</label>
				<input name="gender" type="radio" /><label>Male</label>
				<input name="gender" type="radio" /><label>Other/No Answer</label><br/>
			<br/>
			<label class="title">MQP:</label><br/>
			<a href="group.html">ProjectSpot</a><br/>
			<a href="group.html">Coding D&D for Teens</a><br/>
			<br/>
			<label class="title">Interests:</label><br/>
			<a href="interests.html"><button class="edit">Edit Interests</button></a>
			
			<label class="title">Description:</label>
			<textarea rows=5 cols=50>I like graphics, and I really want to work with upbeat, spirited people who are passionate about the Web!
			</textarea>
			<br/>
			<label class="title">Webpage:</label><br/>
			<label class="title">Office:</label><label>2014</label><br/>
			<label class="title">Contact:</label><label>mcoryea14@wpi.edu</label>
			
			<div class="button_panel">
				<a href="advisor.html"><button class="cancel">Cancel</button></a>
				<a href="advisor.html"><button class="edit">Save</button></a>
			</div>
		</div>
	</div><!--right column-->